...
class GetSearchAnalyticsRequestBodyTests(unittest.TestCase):
    ...
    def test_dimensions_as_json_string_is_normalized_to_list(self):
        # MCP transports can ship list params as JSON strings.
        result = gsc_mcp_server.get_search_analytics(dimensions='["page"]')
        self.assertNotIn("error", result, msg=result)
        self.assertEqual(self.captured_body["dimensions"], ["page"])
        self.assertIsInstance(self.captured_body["dimensions"], list)

    def test_multi_dimension_json_string_is_normalized(self):
        result = gsc_mcp_server.get_search_analytics(dimensions='["page","query"]')
        self.assertNotIn("error", result, msg=result)
        self.assertEqual(self.captured_body["dimensions"], ["page", "query"])

    def test_dimensions_as_comma_separated_string_is_normalized(self):
        result = gsc_mcp_server.get_search_analytics(dimensions="page, query")
        self.assertNotIn("error", result, msg=result)
        self.assertEqual(self.captured_body["dimensions"], ["page", "query"])

    def test_filters_as_json_string_assembles_dimensionFilterGroups(self):
        filters_json = '[{"dimension":"country","operator":"equals","expression":"usa"}]'
        result = gsc_mcp_server.get_search_analytics(
            dimensions=["page"], filters=filters_json
        )
        self.assertNotIn("error", result, msg=result)
        groups = self.captured_body.get("dimensionFilterGroups")
        self.assertIsNotNone(groups)
        self.assertEqual(groups[0]["filters"][0]["dimension"], "country")
        self.assertEqual(groups[0]["filters"][0]["expression"], "usa")
